...
from cogs._bounties_config import (
    ...
    CLAIM_TEMPLATE, UNCLAIM_TEMPLATE, SUBMIT_TEMPLATE,
    APPROVE_TEMPLATE, REJECT_TEMPLATE, PAID_TEMPLATE,
...
class _PostBountyModal(discord.ui.Modal, title="Post a Bounty"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        bounty_id = _db_create(
            self.cog.bot.db,  # type: ignore[attr-defined]
...
def _get_cog(interaction: discord.Interaction) -> "Bounties | None":
    bot = interaction.client
    cog = bot.get_cog("Bounties") if isinstance(bot, commands.Bot) else None
    return cog if isinstance(cog, Bounties) else None
...
# Some bounties (e.g. daily Energy Core) pay different amounts based on what
# the player actually delivered. The scale is stored in two places:
#   * Default per-bounty-type scale at ``bounty_<type>_tiers`` (JSON).
#   * Per-bounty snapshot at ``bounty_tier_scale:<id>`` (JSON), captured at
#     post time so retroactive config edits don't change historical payouts.
# Each tier entry: {"name": "Green", "emoji": "🟢", "silver": 250000}.
...
def _format_tier_scale(tiers: list[dict]) -> str:
    """Bullet list of tier → silver for embedding in a bounty description."""
    out = []
    for t in tiers:
        emoji = t.get("emoji", "•")
        name = t.get("name", "?")
...
def _load_bounty_tier_scale(db, bounty_id: int) -> list[dict] | None:
    """Return the tier scale snapshot saved for this bounty, or None if
    not a tiered bounty. Bad JSON → None (treated as non-tiered)."""
...
def _save_bounty_tier_scale(db, bounty_id: int, tiers: list[dict]) -> None:
    db.set_config(
        f"bounty_tier_scale:{int(bounty_id)}",
        json.dumps([
            {"name": str(t["name"]), "emoji": str(t.get("emoji", "")),
             "silver": int(t.get("silver", 0))}
            for t in tiers if t.get("name")
        ]),
    )


def _load_default_tiers(db, type_key: str, fallback: list[dict]) -> list[dict]:
    """Load the per-type default scale (e.g. ``daily_energy``) from config,
...
class BountySubmitButton(
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if cog is not None:
            try:
                b = _db_get(cog.bot.db, self.bounty_id)  # type: ignore[attr-defined]
                title = (b or {}).get("title", "") or ""
                is_sso = title.startswith(Bounties.SSO_TITLE_PREFIX)
            except Exception:
                is_sso = False
        if is_sso:
