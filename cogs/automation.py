...
# ── Auto-kick: members who never verify ─────────────────────────────────────
#
...
# longer than ``automation_unverified_kick_days`` (default 7).
#
# Gated by ``automation_unverified_kick_enabled`` (default 0 = disabled) so
# this never fires until an officer explicitly turns it on.
#
# Safety rails:
#   * never kick bots
...
def _collect_unverified_kick_targets(
    ...
    """Return [(member, age_days), ...] for members eligible to be kicked
    for being Unverified > ``days`` days. Sorted oldest-first."""
...
async def _run_unverified_kicks(bot: Bot) -> None:
    ...
    No-op unless ``automation_unverified_kick_enabled`` config is truthy.
    Posts a summary to the officer-tasks channel (if configured).
    """
    enabled = _get_int_config(bot.db, "automation_unverified_kick_enabled", 0)
    if not enabled:
        return
    days = _get_int_config(
        bot.db, "automation_unverified_kick_days", _DEFAULT_UNVERIFIED_KICK_DAYS,
    )
    if days < 1:
    ...
    for guild in bot.guilds:
        ...
        for member, age in targets:
            ...
            # Best-effort DM. Many users have DMs closed; ignore failures.
            try:
                await member.send(
            ...
            try:
                ...
                kicked.append(
                    f"{member} ({member.id}) — {age}d in {guild.name}"
                )
                info_log(
...
#
# Gentle DM reminder for new Discord members who still have the Unverified role
# and have not started/finished registration. This is intentionally softer than
# the auto-kick routine: DM-only, cooldown-gated, and capped per member.
...
async def _run_unverified_nudges(bot: Bot) -> None:
    ...
    min_days = _get_int_config(
        bot.db, "automation_unverified_nudge_days",
        _DEFAULT_UNVERIFIED_NUDGE_DAYS,
    )
    cooldown_days = _get_int_config(
        bot.db, "automation_unverified_nudge_cooldown_days",
        _DEFAULT_UNVERIFIED_NUDGE_COOLDOWN_DAYS,
    )
    max_count = _get_int_config(
        bot.db, "automation_unverified_nudge_max",
        _DEFAULT_UNVERIFIED_NUDGE_MAX,
    )
    ...
    for guild in bot.guilds:
        ...
        for member in list(role.members):
            ...
            try:
                ...
                nudged.append(
                    f"{member.mention} (`{member}`) — {age}d unverified, nudge {prior_count + 1}/{max_count}"
                )
            except discord.Forbidden:
                bot.db.mark_unverified_nudge_sent(str(member.id), today_iso)
